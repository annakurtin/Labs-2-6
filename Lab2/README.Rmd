...
*When you knit to a github_document, it will generate in your GitHub repo a .md file (no code just the output (?))*
...
```{r Install and Read Packages Function}
...
# specify to use the plot function from the base package
...
##geom_sf allows you to interface with sf
# Note - color = NA in the geom_sf() removes the border lines
moose_plot<-ggplot() + geom_sf(data = elc_habitat, mapping = aes(fill = as.factor(MOOSE_W)), color = NA) + labs(x="Easting",y="Northing") + theme(axis.text.y = element_text(angle = 90, hjust=0.5))
moose_plot
...
#adjust fill colors of SHEEP_W  (note that I just selected some random colors, but made "7" as blue)
...
Why rasterize vector files? To make a pretty map and because you can't do math with a vector
...
## interacts with ggplot well
## also used for random forest models 
...
## use this when you do a bunch of raster calculations and want to come back to this
```
...
```{r}
# check the extent of a raster with ext(elevation2)
...
# you can see from the extents that these aren't aligned
ext(elevation2)
ext(disthighhumanaccess)
# resampling is done with bilinear method - easy to do with linear/numerical data that is continuous
# resampling with categorical variables is more complicated
```
...
# allows you to have an interactive window for your maps
tmap_mode("view")
...
## Part 1c - Calculating Distance to Raster (Human Access Layer)
...
# we also need to clean up our spellings
...
class(rd)
# Fit 99% mpc
## mcp ensure you have at least 5 locations to evaluate a home range size
...
Next we will fit a 99% Minimum Convex Polygon to these cleaned data for EACH individual 

- old method that has been around for a long time

- MCP not a statistical estimator and not based in math
...
## How many gps points do you need in order to estimate home range size effectively? 
mcp.area(rd, percent=seq(50, 100, by=5))

#Rarefaction curves -coming up later
```


We will now estimate Kernel Density Estimate home ranges using the command `kernelUD()`.

- a way of estimating home range sizes that has a base in probability theory 

- gives rasters as an output (you can do mathmatical calculations)
```{r, warning= FALSE}
...
Example of manipulating a joint UD. 
```{r}
...
For kernel estimating methods, you have to do the utilization distribution based on each units and then add them together. Don't calculate both of them at the same time because they are all scaled relative to the sample size and if your sample sizes are different the scaling will be off 
...
# crs(layer) gives you the coordinate system
# if this throws an error
# crs(elevation2) <- proj4 string
...
## extract() extracts values from a spatial raster stack 
...
wolfused <- merge(rdused, bvused, all.x= TRUE, all.y = TRUE) #sometimes need to be cautious with merged
str(wolfused)
head(wolfused)

# Create a field that keeps track of the used locations
...
These are wolves outisde the pack or in areas outside of the human access layers

*Usually you would go and fix these before moving on*
```{r}
...
wolfused <- na.omit(wolfused) # get rid of every NA in the wolf dataset in any column 
summary(wolfused)
dim(wolfused)
```
...
Look at a simple analyses of use
```{r}
par(mfrow = c(2,3))
